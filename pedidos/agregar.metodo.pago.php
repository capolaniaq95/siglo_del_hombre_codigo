            ...
            <div class="container mt-4">
                ...
                if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                    ...
                    if ($mysqli->query($sql) === TRUE) {
                        echo "<div class='alert alert-success'>metodo agregado correctamente.</div>";
                        echo "<a href='metodo.pago.php' class='btn btn-primary'>Volver a la lista de metodos de pago</a>";
                        $mysqli->close();
                        exit;
                    } else {
                        echo "<div class='alert alert-danger'>Error al agregar el metodo: " . $mysqli->error . "</div>";
                    }
                }
                ?>
                <form action="" method="post">
                    <div class="form-group">
                        <label for="ubicacion">Nombre del metodo de pago</label>
                        <input type="text" class="form-control" id="ubicacion" name="metodo" required>
                    </div>
                    <div class="form-group d-flex">
                        <button type="submit" class="btn btn-info mr-2">Agregar metodo</button>
